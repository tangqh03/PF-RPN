    ...
    def loss_by_feat(
        ...
        if all_layers_outputs_conf is not None:
            if router_weights is not None:
                losses_cls, losses_bbox, losses_iou, losses_ctr, losses_rt = multi_apply(
                ...
                loss_dict['loss_ctr'] = losses_ctr[-1]
            else:
                losses_cls, losses_bbox, losses_iou, losses_ctr = multi_apply(
                    self.loss_by_feat_single,
                    all_layers_cls_scores,
                    all_layers_bbox_preds,
                    all_layers_outputs_conf,
            ...
            num_layers = len(all_layers_cls_scores)
            if router_weights is not None:
                losses_cls, losses_bbox, losses_iou, losses_rt = multi_apply(
                    self.loss_by_feat_single,
                    all_layers_cls_scores,
                    all_layers_bbox_preds,
                    [None]*num_layers,
        ...
        num_dec_layer = 0
        if all_layers_outputs_conf is not None:
            for loss_cls_i, loss_bbox_i, loss_iou_i, loss_ctr_i in \
                    zip(losses_cls[:-1], losses_bbox[:-1], losses_iou[:-1],losses_ctr[:-1]):
                loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
                loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
                loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
                loss_dict[f'd{num_dec_layer}.loss_ctr'] = loss_ctr_i
                num_dec_layer += 1
        else:
            for loss_cls_i, loss_bbox_i, loss_iou_i in \
                ...
                num_dec_layer += 1
        if router_weights is not None:
            for loss_rt_i in losses_rt[:-1]:
                loss_dict[f'd{num_dec_layer}.loss_rt'] = loss_rt_i
        return loss_dict
