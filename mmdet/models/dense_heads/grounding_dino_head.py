...
def convert_grounding_to_cls_scores(logits: Tensor,
    ...
    """Convert token-level logits to class-level scores using positive maps."""
    ...
    if positive_maps is not None:
        if all(x == positive_maps[0] for x in positive_maps):
            ...
            for label_j in positive_map:
                token_ids = torch.LongTensor(positive_map[label_j]).to(
                    logits.device)
                scores[:, :, label_j - 1] = logits[:, :, token_ids].mean(-1)
        else:
            for i, positive_map in enumerate(positive_maps):
                for label_j in positive_map:
                    token_ids = torch.LongTensor(positive_map[label_j]).to(
                        logits.device)
                    scores[i, :, label_j - 1] = logits[i, :, token_ids].mean(
                        -1)
    return scores
...
class GroundingDINOHead(DINOHead):
    ...
    def __init__(self, contrastive_cfg=dict(max_text_len=256), loss_ctr:ConfigType = dict(type='L1Loss', loss_weight=5.0), rm_lang_embed: bool=False, use_conf_branch: bool=False, output_score: str='both', **kwargs):
        self.contrastive_cfg = contrastive_cfg
        self.rm_lang_embed = rm_lang_embed
        self.use_conf_branch = use_conf_branch
        self.output_score = output_score
        self.max_text_len = contrastive_cfg.get('max_text_len', 256)
        super().__init__(**kwargs)
        self.loss_ctr = MODELS.build(loss_ctr)
    ...
    def _init_layers(self) -> None:
        ...
        if self.use_conf_branch:
            conf_branch = nn.Sequential(
                Linear(self.embed_dims, self.embed_dims),
                nn.ReLU(),
                Linear(self.embed_dims, 1)
            )
            if self.share_pred_layer:
                self.conf_branches = nn.ModuleList([conf_branch for _ in range(self.num_pred_layer)])
            else:
                self.conf_branches = nn.ModuleList([copy.deepcopy(conf_branch) for _ in range(self.num_pred_layer)])
    ...
    def forward(
        ...
        for layer_id in range(hidden_states.shape[0]):
            ...
            if self.use_conf_branch:
                tmp_conf_preds = self.conf_branches[layer_id](hidden_state)
                conf_score = tmp_conf_preds.sigmoid()
                all_layers_outputs_conf.append(conf_score)
        ...
        if self.use_conf_branch:
            all_layers_outputs_conf = torch.stack(all_layers_outputs_conf)
            return all_layers_outputs_classes, all_layers_outputs_coords, all_layers_outputs_conf
        else:
            return all_layers_outputs_classes, all_layers_outputs_coords,None
    ...
    def predict(self,
                ...
                rescale: bool = True,
                router_weights:Tensor = None) -> InstanceList:
    ...
    def predict_by_feat(self,
        ...
        if all_layers_confs is not None:
            conf_preds = all_layers_confs[-1]
        else:
            conf_preds = all_layers_confs
        result_list = []
        for img_id in range(len(batch_img_metas)):
            ...
            if conf_preds is not None:
                conf_pred = conf_preds[img_id]
            else:
                conf_pred = None
            token_positive_maps = batch_token_positive_maps[img_id]
            results = self._predict_by_feat_single(cls_score, bbox_pred,conf_pred,
                                                   token_positive_maps,
                                                   img_meta, rescale)
    ...
    def _predict_by_feat_single(self,
        ...
        if token_positive_maps is None or self.rm_lang_embed:
            cls_score = cls_score.sigmoid()
            scores, _ = cls_score.max(-1)
            if conf_pred is not None:
                conf_pred = conf_pred.squeeze(-1).sigmoid()
                if self.output_score == 'both':
                    scores = conf_pred + scores
                elif self.output_score == 'cls':
                    scores = scores
                elif self.output_score == 'fuse':
                    scores = scores ** 0.4 * conf_pred ** 0.6
                else:
                    raise ValueError(f"Invalid output_score: {self.output_score}")
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
             ...
             enc_outputs_class: Tensor, enc_outputs_coord: Tensor,enc_outputs_conf:Tensor,
             batch_data_samples: SampleList, dn_meta: Dict[str, int], router_weights:Tensor = None) -> dict:
        """Perform forward propagation and loss calculation of the detection
        head on the queries of the upstream network.
    ...
    Returns:
        ...
        for data_sample in batch_data_samples:
            ...
            if self.rm_lang_embed:
                data_sample.gt_instances.positive_maps = torch.ones_like(
                    data_sample.gt_instances.positive_maps,
                    device=data_sample.gt_instances.positive_maps.device)
                data_sample.gt_instances.positive_maps[:, 0] = 0
                data_sample.gt_instances.positive_maps[:, -1] = 0
            batch_gt_instances.append(data_sample.gt_instances)

        outs = self(hidden_states, references, memory_text, text_token_mask)
        self.text_masks = text_token_mask
        loss_inputs = outs + (enc_outputs_class, enc_outputs_coord,enc_outputs_conf,
                              batch_gt_instances, batch_img_metas, dn_meta,router_weights)
        losses = self.loss_by_feat(*loss_inputs)
        return losses

    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,conf_pred: Tensor,
                            batch_gt_instances: InstanceList,
                            batch_img_metas: List[dict],router_weights:Tensor = None) -> Tuple[Tensor]:
        ...
        if self.use_conf_branch:
            with torch.no_grad():
                pos_mask = (bbox_weights.sum(dim=1) > 0)            # [bs*nq], True 表示正样本
                num_pos = max(int(pos_mask.sum().item()), 1)
                pred_xyxy_pos = bboxes[pos_mask]                    # [P, 4]  (x1,y1,x2,y2)
                gt_xyxy_pos   = bboxes_gt[pos_mask]                 # [P, 4]

                # 预测中心
                pred_ctr = 0.5 * (pred_xyxy_pos[:, :2] + pred_xyxy_pos[:, 2:])  # [P, 2] (cx, cy)

                # 与 GT 四边的距离（ltrb）
                l = (pred_ctr[:, 0] - gt_xyxy_pos[:, 0]).clamp(min=0)
                t = (pred_ctr[:, 1] - gt_xyxy_pos[:, 1]).clamp(min=0)
                r = (gt_xyxy_pos[:, 2] - pred_ctr[:, 0]).clamp(min=0)
                b = (gt_xyxy_pos[:, 3] - pred_ctr[:, 1]).clamp(min=0)

                # 计算 centerness target ∈ [0,1]
                tb_min = torch.min(t, b)
                tb_max = torch.max(t, b) + 1e-12
                lr_min = torch.min(l, r)
                lr_max = torch.max(l, r) + 1e-12
                centerness_target = torch.sqrt((tb_min / tb_max) * (lr_min / lr_max)) 
                # centerness_full = torch.zeros_like(pos_mask, dtype=centerness_target.dtype)
                # centerness_full[pos_mask] = centerness_target
                centerness_full = torch.zeros_like(pos_mask, dtype=centerness_target.dtype).masked_scatter(pos_mask, centerness_target)
            conf_pred = conf_pred.reshape(-1)
            ctr_weight = pos_mask.long()
            loss_ctr = self.loss_ctr(
                    conf_pred, centerness_full, ctr_weight, avg_factor=num_total_pos)
            if router_weights is not None:
                loss_rt = torch.std(router_weights)
                return loss_cls, loss_bbox, loss_iou,loss_ctr,loss_rt
            else:
                return loss_cls, loss_bbox, loss_iou,loss_ctr
        else:
            if router_weights is not None:
                loss_rt = torch.std(router_weights)
                return loss_cls, loss_bbox, loss_iou,loss_rt
            else:
                return loss_cls, loss_bbox, loss_iou

    def _loss_dn_single(self, dn_cls_scores: Tensor, dn_bbox_preds: Tensor,conf_pred:Tensor,
                        batch_gt_instances: InstanceList,
                        batch_img_metas: List[dict],
