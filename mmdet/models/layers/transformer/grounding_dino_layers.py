...
class GroundingDinoTransformerEncoder(DeformableDetrTransformerEncoder):
    ...
    def __init__(self, text_layer_cfg: ConfigType,
                 fusion_layer_cfg: ConfigType, spp: str=None, mask_thr: float=0.5, **kwargs) -> None:
        self.text_layer_cfg = text_layer_cfg
        self.fusion_layer_cfg = fusion_layer_cfg
        self.spp = spp  # self-prompt projector
    ...
    def _init_layers(self) -> None:
        ...
        # Self-prompt projector
        if self.spp:
            # self.spatial_prompt = []
            # self.channel_prompt = []
            # for _ in range(self.num_layers):
            #     spatial_prompt = torch.nn.parameter.Parameter(torch.randn(self.embed_dims), requires_grad=True)
            #     channel_prompt = torch.nn.parameter.Parameter(torch.eye(self.embed_dims), requires_grad=True)
            #     self.spatial_prompt.append(spatial_prompt)
            #     self.channel_prompt.append(channel_prompt)
            self.spatial_prompt = nn.ParameterList()
            self.channel_prompt = nn.ParameterList()
            for _ in range(self.num_layers):
                spatial_prompt = torch.nn.parameter.Parameter(torch.randn(self.embed_dims), requires_grad=True)
                channel_prompt = torch.nn.parameter.Parameter(torch.eye(self.embed_dims), requires_grad=True)
                self.spatial_prompt.append(spatial_prompt)
    ...
    def forward(self,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            output = layer(
                ...
                key_padding_mask=key_padding_mask)
            if self.spp:
                if self.spp != 'text-self-prompt':
                    cos_sim = F.normalize(output, dim=-1) @ F.normalize(self.spatial_prompt[layer_id][None, ..., None], dim=-1)
                    mask = cos_sim.clamp(0, 1) > self.mask_thr
                    mask_f = mask.float()
                    if self.spp == 'vision':
                        x = output * mask_f
                        prompt_res = x @ self.channel_prompt[layer_id]
                        output = output + prompt_res
                    elif self.spp == 'text':
                        sum_masked = (output * mask_f).sum(dim=1, keepdim=True)
                        count = mask_f.sum(dim=1, keepdim=True).clamp(min=1e-6)
                        x = sum_masked / count
                        memory_text[:, [1], :] += x
                    elif self.spp == 'self-prompt':
                        x = output * mask_f
                        prompt_res = x @ self.channel_prompt[layer_id]
                        output = output + prompt_res
                        output = layer(
                        ...
                        raise NotImplementedError(f'SPP {self.spp} not implemented.')
                else:
                    cos_sim = F.normalize(output, dim=-1) @ F.normalize(memory_text[:, [1], :].permute(0, 2, 1), dim=-1)
                    mask = cos_sim.clamp(0, 1) > self.mask_thr
                    mask_f = mask.float()
                    sum_masked = (output * mask_f).sum(dim=1, keepdim=True)
                    count = mask_f.sum(dim=1, keepdim=True).clamp(min=1e-6)
